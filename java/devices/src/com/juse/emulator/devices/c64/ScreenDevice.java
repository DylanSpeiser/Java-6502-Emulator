...
import com.juse.emulator.util.fonts.FontManager;
...
public class ScreenDevice extends JFrame implements BusDevice, HasPorts, ActionListener
{
	...
	public ScreenDevice(int baseAddress, int displayColumns, int displayRows)
	{
		...
		this.bar = new BusAddressRangeImpl(baseAddress,
									   bankSize,
				                       1);
		...
		ports[CONST_COLORPAGE]   = new TextPort("TEXT-COLORPAGE",bar.getHighAddress()+1,
				                                new BusAddressRangeImpl(bar.getHighAddress()+1,bankSize,1), this);
		ports[CONST_PALETTEPORT] = new TextPort("TEXT-PALETTEPORT",bar.getHighAddress()+bankSize+1,
				                                new BusAddressRangeImpl(bar.getHighAddress()+bankSize+1,3 + 1,1), this); 			
	...
	public BusDevice[] ports(int baseAddress)
	{
		BusDevice[] ports = 
		{
			//new TextPort("TEXT-COLORPAGE",bar.getHighAddress()+1,new BusAddressRangeImpl(bar.getHighAddress()+1,bankSize,1), this),
			//new TextPort("TEXT-PALETTEPORT",bar.getHighAddress()+bankSize+1,new BusAddressRangeImpl(bar.getHighAddress()+bankSize+1,3 + 1,1), this),
			new TextPort("C64-COLORPAGE",0xD800,new BusAddressRangeImpl(0xD800,0x03E7,1), this),
		};
		return ports;
	}	
