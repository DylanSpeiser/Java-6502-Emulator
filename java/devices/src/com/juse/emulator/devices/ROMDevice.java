...
import com.juse.emulator.interfaces.ROM;
import com.juse.emulator.util.loaders.ROMManager;

public class ROMDevice implements BusDevice, ROM
{
	private byte[] bank;
	private BusAddressRange bar;

	public ROMDevice(BusAddressRange bar, byte[] romImage)
	{
	...
	{
		this(new BusAddressRangeImpl(bankAddress, romImage.length - 1, 1), romImage);
	}
	...
	{
		this(new BusAddressRangeImpl(bankAddress, 1024, 1), new byte[1024]);
	}
	
	public ROMDevice()
	{
		//this(new BusAddressRangeImpl(bankAddress, bankSize, 1), romImage);
	}
	...
	public static void main(String[] args)
	{
		...
		byte[] rom = ROMManager.loadROM("demo.rom");
		
		ROMDevice rd = new ROMDevice(0x00008000, rom);
